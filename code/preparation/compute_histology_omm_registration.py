...
def compute_histology_omm_registration():
    """
    Updates histology ROI information with OMM coordinates (both physical
    and voxel coordinates).

    Registration uses a two-dimensional tissue mask per histology donor and a
    two-dimensional fractional anisotropy (FA) template in OMM space (thresholded at FA >= 0.6)
    ...
    cc_atlas_mask_file = os.path.join(
        ...
        "space-omm_desc-FAThresholded2D_mask.nii.gz")
    ...
    omm_roi_coordinates_file = os.path.realpath(
        f"{histo_raw_data_path}/desc-noOmm_roiinfo.tsv")
    roi_info_df = pd.read_csv(
        omm_roi_coordinates_file, sep="\t")

    # allocate array for OMM voxel coordinates
    ...
    for subject_path in subject_paths:
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            # estimate registration: histology <-> OMM space
            run_command(
                ...
                f"_from-native_to-omm_desc-initial_affine.mat"
            )

            # transform histology tissue mask to OMM space
            run_command(
        ...
        # compute physical coordinates in OMM space
        ...
        # add OMM coordinates to ROI info table
        roi_info_df["omm_voxel_x"] = coords_voxel_3d[:, 0]
        roi_info_df["omm_voxel_y"] = coords_voxel_3d[:, 1]
        roi_info_df["omm_voxel_z"] = coords_voxel_3d[:, 2]
        roi_info_df["omm_physical_x"] = coords_physical_3d[:, 0]
        roi_info_df["omm_physical_y"] = coords_physical_3d[:, 1]
        roi_info_df["omm_physical_z"] = coords_physical_3d[:, 2]
...
if __name__ == "__main__":
    compute_histology_omm_registration()
